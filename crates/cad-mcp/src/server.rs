...
impl GfpCadMcpServer {
    ...
    async fn set_grid(&self, Parameters(i): Parameters<SetGridInput>) -> String {
        ...
        let Some(b) = &mut s.building else {
            return NO_BLDG.into();
        };
        b.grid.x_axes = i
            .x_axes
            .iter()
            .map(|a| GridAxis::new(&a.name, a.position))
            .collect();
        b.grid.y_axes = i
            .y_axes
            .iter()
    ...
    async fn add_opening(&self, Parameters(i): Parameters<AddOpeningInput>) -> String {
        ...
        let Ok(wid) = uuid::Uuid::parse_str(&i.wall_id) else {
            return "Error: Invalid wall_id".into();
        };
        let sill = i.sill_height.unwrap_or(0.0);
        let mut op = if sill > 0.0 {
            Opening::window(wid, i.position, i.width, i.height, sill)
        } else {
            Opening::door(wid, i.position, i.width, i.height)
        };
        if let Some(k) = &i.kind {
            op.kind = parse_opening_kind(k);
        }
    ...
    async fn export_dxf(&self, Parameters(i): Parameters<ExportDxfInput>) -> String {
        ...
        match std::fs::File::create(&i.path) {
            Ok(mut f) => match cad_dxf::DxfExporter::new(enc).export(b, &mut f) {
                Ok(r) => format!("Exported: {r}"),
    ...
    async fn render_autocad(&self, Parameters(i): Parameters<RenderAutocadInput>) -> String {
        ...
        match cad_acad::AcadRenderer::with_origin(origin).render_building(b) {
            Ok(r) => format!("{r}"),
            Err(e) => format!("Error: {e}"),
        }
    }
    ...
    #[tool(
        name = "login",
        description = "Auth0 ログイン（ローカル専用。本番ではJWT検証が必要）"
    )]
    async fn login(&self, Parameters(i): Parameters<LoginInput>) -> String {
            ...
            .await
        {
            Ok(user) => {
                ...
                let id = user.id.as_ref().map(rid_to_string).unwrap_or_default();
    ...
    async fn list_workspaces(&self, Parameters(_): Parameters<EmptyInput>) -> String {
        ...
        let Some(user) = user else {
            return NO_LOGIN.into();
        };
        match db.list_workspaces(&user).await {
            Ok(ws) => {
                let mut o = format!("{} workspaces:\n", ws.len());
                for w in &ws {
                    o += &format!(
                        "  {} ({})\n",
                        w.name,
                        w.id.as_ref().map(rid_to_string).unwrap_or_default()
                    );
                }
                o
            }
    ...
    async fn create_project(&self, Parameters(i): Parameters<NameInput>) -> String {
        ...
        let (user, ws_id) = {
            let s = self.state.lock().await;
            (s.current_user.clone(), s.current_workspace.clone())
        };
        let (Some(user), Some(ws_id)) = (user, ws_id) else {
            return "Error: Login and select workspace".into();
        };
        match db.create_project(&ws_id, &i.name, &user).await {
            Ok(p) => {
                let id = p.id.clone();
                self.state.lock().await.current_project = id.clone();
                format!(
                    "Project '{}' ({})",
                    i.name,
                    id.map(|r| rid_to_string(&r)).unwrap_or_default()
                )
            }
            Err(e) => format!("Error: {e}"),
        }
    ...
    async fn list_projects(&self, Parameters(_): Parameters<EmptyInput>) -> String {
        ...
        let Some(ws_id) = ws_id else {
            return "Error: Select workspace".into();
        };
        match db.list_projects(&ws_id).await {
            Ok(ps) => {
                let mut o = format!("{} projects:\n", ps.len());
                for p in &ps {
                    o += &format!(
                        "  {} ({})\n",
                        p.name,
                        p.id.as_ref().map(rid_to_string).unwrap_or_default()
                    );
                }
                o
            }
    ...
    async fn save_building(&self, Parameters(_): Parameters<EmptyInput>) -> String {
        ...
        let (building, user, proj_id) = {
            ...
            (
                s.building.clone(),
                s.current_user.clone(),
                s.current_project.clone(),
            )
        };
        let (Some(building), Some(user), Some(proj_id)) = (building, user, proj_id) else {
            return "Error: Need building + login + project".into();
        };
        match db.save_building(&proj_id, &building, &user).await {
            Ok(r) => format!(
                "Saved '{}' ({})",
                building.name,
                r.id.map(|r| rid_to_string(&r)).unwrap_or_default()
            ),
            Err(e) => format!("Error: {e}"),
        }
    }
    ...
    async fn list_buildings(&self, Parameters(_): Parameters<EmptyInput>) -> String {
        ...
        let Some(proj_id) = proj_id else {
            return "Error: Select project".into();
        };
        match db.list_buildings(&proj_id).await {
            Ok(bs) => {
                let mut o = format!("{} buildings:\n", bs.len());
                for b in &bs {
                    o += &format!(
                        "  {} ({})\n",
                        b.name,
                        b.id.as_ref().map(rid_to_string).unwrap_or_default()
                    );
                }
                o
            }
